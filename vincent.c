...
int i,j;
    ...
    act = vincentData[code];

    for (i = 0; i < CHAR_HEIGHT; i++)
    {
        for (j = 0; j < CHAR_WIDTH; j++)
        {
        char mask = 0x01, actChar = act[i], maskResult, offset = CHAR_WIDTH - j - 1;

            mask = mask << offset;

            maskResult = (actChar & mask) >> offset;

            ris.pixelMatrix[i][j] = maskResult;
        }
    }
